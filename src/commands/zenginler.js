...
let data = await economy.find().sort([['coinsInWallet', 'descending']])
    ...
    if (data.length == 0) return message.channel.send('Sunucuda zengin yok'); 
    
    const emojis = [':first_place:', ':second_place:', ':third_place:'];
    data = data.map((x, i) => `${emojis[i] || '🔹'} **${x.coinsInWallet.toLocaleString()}** - ${bot.users.cache.get(x.userId).tag || 'Feri#0000'}`);

    const embed = new MessageEmbed()
        .setAuthor(`${message.guild.name} sunucusundaki zenginler`)
