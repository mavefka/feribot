...
module.exports.run = async (bot, message, args) => {
    ...
    if (item.length < 1) {
        return message.channel.send('Eşyan yok.');
    }
    const items = item.map(x => `${x.description}\n  \`id: ${x.name} \` x ${x.amount.toLocaleString()}`);
    const embed = new MessageEmbed()
        .setThumbnail(member.user.displayAvatarURL({ format: 'png', size: 256, dynamic: true }))
        .setFooter("aaaaaaaaaaaaaaaa")
        .setTitle(`${message.author.username}'nın çantası`)
        .setDescription(`${items.join('\n\n')}`)
        .setFooter(`Sayfa ${args[0] || 1}  ${page}`)
        .setColor('RANDOM');
