...
module.exports.run = async (bot, message, args) => {
    ...
    .setColor("RED")
    .setDescription(`${x} **${usertag.user.username}** : Kime?, niye nasıl?`);
    ...
    .setColor("RED")
      .setDescription(`${x} **${usertag.user.username}** : Şakamısın?.`);
   ...
   //return message.channel.send(`Lol you can't give yourself coins u crazy.`);
    ...
    .setColor("RED")
    .setDescription(`${x} **${usertag.user.username}** : .......`);
    ...
    if (isNaN(args[1]) && args[1] != 'all') {
        return message.channel.send(`olmaz`)
    }
    ...
    let sendcoinsembed3 = new MessageEmbed()
    .setColor("GREEN")
    .setDescription(`${tick} ${member} kişisine **${parseInt(toGive).toLocaleString()}** tl verdin!`);
    ...
    let sendcoinsembed3 = new MessageEmbed()
    .setColor("GREEN")
    .setDescription(`${tick} ${usertag.user.username} : ${member} kişisine **${parseInt(toGive).toLocaleString()}** tl verildi.`);
    message.channel.send(sendcoinsembed3).catch();
    }
